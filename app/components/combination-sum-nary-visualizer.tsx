...
type TreeNode = {
  ...
  currentIndex: number;    // loop starts from this index
  combination: number[];   // combination so far
  remaining: number;       // target remaining
  status: NodeStatus;
  edgeLabel: string;       // "pick 2" | "pick 3" | ...
  children: number[];
};

type Tree = Record<number, TreeNode>;

// Pre-build the full N-ary recursion tree
let nextId = 0;
function buildTree(
  candidates: number[],
  target: number,
  ...
  function dfs(
    ...
    else if (remaining < 0) status = "pruned";
    ...
    // Recurse: for loop from currentIndex to end
    if (status === "pending") {
      for (let i = currentIndex; i < candidates.length; i++) {
        const c = candidates[i];
        dfs(id, i, [...combination, c], remaining - c, `pick ${c}`);
      }
    }
  }
...
export default function CombinationSumNaryVisualizer() {
  ...
  return (
    <div className="mt-5 mb-10 border border-gray-200 rounded-2xl overflow-hidden">
          ...
          {activeNode ? (
            <div>
              <span className="font-mono font-semibold text-indigo-700">
                F({activeNode.currentIndex}, [{candidates.join(",")}], {activeNode.remaining})
              </span>
              {" — "}
              ...
              {(activeNode.status === "pending" || activeNode.status === "active") && (
                <span className="text-gray-600">
                  ...
                  <span className="font-mono">
                    i = {activeNode.currentIndex} → {candidates.length - 1}
                  </span>
                  , trying [{candidates.slice(activeNode.currentIndex).join(", ")}]
                </span>
              )}
            </div>
          ) : (
      ...
      <div className="overflow-x-auto bg-white p-4">
        ...
        >
          <defs>
            <marker id="arrow-nary" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
            ...
            return (
              <g key={`node-${node.id}`}>
                ...
                <text
                  ...
                  fill={s.text}
                >
                  {node.combination.length > 0 ? `[${node.combination.join(",")}]` : "[ ]"}
                </text>
                {/* remaining */}
                <text
                  x={pos.x + NODE_W / 2}
