...
destination where logs are stored/printed. 

1. Whenever an application wants to log an event, it **calls the logger** with a **message** 
and an appropriate **severity level**. 

2. The logger creates a **log record** containing information such as **timestamp**, **severity level**, 
**message**, and **emitting thread name**.

3. The **log record** is then passed to the logging pipeline, where it is first checked 
against the configured log level. For example, if the application is configured to log only
`INFO` and above, `DEBUG` messages will be discarded immediately. Valid logs are then 
passed to the configured destinations (commomly called **appenders**) such as **console**,
**file**, **database**, or **remote logging system**.

<LoggingSequenceVisualizer />

---

### Object-Model Design

(continue second iteration from here...)

---

In a production system, this processing is often performed asynchronously. Instead of 
making the application wait while a log is written to a file or sent over the network, the 
logger places the record into an in-memory queue. A background worker consumes records from 
the queue, formats or serializes them as required, and sends them to the configured 
destinations.
