...
Let `F(combination, currentIndex, candidates, target)` be the function that returns all valid combinations. 


This function is guaranteed to generate all unique combinations that sum to the `target` by exploring every 
feasible choice through recursion.
